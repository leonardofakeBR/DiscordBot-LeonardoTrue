...
class Musica(commands.Cog):
    ...
    async def play(self, interaction:discord.Interaction, musica:str, loop:bool):
        ...
        songs_queues[guild_id].append((audio_url, title))
        ...
        if voice_client.is_playing() or voice_client.is_paused():
            ...
            await play_next_song(self.bot, voice_client, guild_id, interaction.channel, loop)
...
async def play_next_song(bot, voice_client, guild_id, channel, loop):

    if songs_queues[guild_id]:
        if loop:
            audio_url, title = songs_queues[guild_id][0]
        else:
            audio_url, title = songs_queues[guild_id].popleft()
        ffmpeg_options = {
        ...
        def after_play(error):
            ...
            asyncio.run_coroutine_threadsafe(play_next_song(bot, voice_client, guild_id, channel, loop), bot.loop)
